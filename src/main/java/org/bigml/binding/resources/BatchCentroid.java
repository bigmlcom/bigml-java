...
/**
 * Entry point to create, retrieve, list, update, and delete batchbatchcentroids.
 *
...
public class BatchCentroid extends AbstractResource {
   ...
   *
   */
  public BatchCentroid(final String apiUser, final String apiKey, final boolean devMode) {
   ...
   *
   * POST /andromeda/batchbatchcentroid?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY; HTTP/1.1
   * Host: bigml.io
   ...
   * @param args		set of parameters for the new batchbatchcentroid. Optional
   * @param waitTime	time (milliseconds) to wait for next check of FINISHED status for batchbatchcentroid
   * 					before to start to create the batchbatchcentroid. Optional
   * @param retries		number of times to try the operation. Optional
   *
   */
  public JSONObject create(final String clusterId, final String datasetId, String args, Integer waitTime, Integer retries) {
	...
	try {
	  ...
	  if (waitTime > 0) {
	     int count = 0;
	     while (count<retries &&
	     !BigMLClient.getInstance(this.devMode).clusterIsReady(clusterId)) {
	  ...
	  return createResource(BATCH_CENTROID_URL, requestObject.toJSONString());
	} catch (Throwable e) {
   ...
   *
   * A batchcentroid is an evolving object that is processed until it
   * reaches the FINISHED or FAULTY state, the method will return a
   * JSONObject that encloses the batchcentroid values and state info
   * available at the time it is called.
   *
   * GET /andromeda/batchbatchcentroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
  ...
  public JSONObject get(final String batchcentroidId) {
    if (batchcentroidId == null || batchcentroidId.length() == 0 || !batchcentroidId.matches(BATCH_CENTROID_RE)) {
   ...
   * HTTP/1.1 Host: bigml.io
   *
   * @param batchcentroid 	a batchcentroid JSONObject.
   *
   */
   ...
   *
   * GET /andromeda/batchbatchcentroid?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * Host: bigml.io
   *
   ...
   *
   * PUT /andromeda/batchbatchcentroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * Content-Type: application/json
   *
   * @param batchcentroidId 	a unique identifier in the form batchcentroid/id where id
   * 					is a string of 24 alpha-numeric chars.
   * @param changes		set of parameters to update the batchcentroid. Optional
   *
   */
   ...
   * Content-Type: application/json
   *
   * @param batchcentroid 	an batchcentroid JSONObject
   * @param changes		set of parameters to update the batchcentroid. Optional
   *
   */
  public JSONObject update(final JSONObject batchcentroid, final JSONObject changes) {
    String resourceId = (String) batchcentroid.get("resource");
   ...
   *
   * DELETE /andromeda/batchbatchcentroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1
   *
