 ...
 *
 * Full API documentation on the API can be found from BigML at: https://bigml.com/developers/ensembles
 *
 *
 */
public class Ensemble extends AbstractResource {
   ...
   *
   */
  public Ensemble(final String apiUser, final String apiKey, final boolean devMode) {
   ...
   *
   * POST /andromeda/ensemble?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY; HTTP/1.1 Host: bigml.io
  ...
  public JSONObject get(final String ensembleId) {
    if (ensembleId == null || ensembleId.length() == 0 || !ensembleId.matches(ENSEMBLE_RE)) {
