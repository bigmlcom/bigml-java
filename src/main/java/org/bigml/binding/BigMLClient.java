...
public class BigMLClient {
  ...
  private void initResources() {
    ...
    batchCentroid = new BatchCentroid(this.bigmlUser, this.bigmlApiKey, this.devMode);
  }
   ...
   *          gzipped, or zipped. Required multipart/form-data; charset=utf-8
   * @param name
   ...
   * 
   * @param sourceId
   *          a unique identifier in the form source/id where id is a string of
   *          24 alpha-numeric chars.
   * 
   */
   ...
   * 
   * POST /andromeda/source/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io Content-Type: application/json
   * 
   * @param sourceId
   *          a unique identifier in the form source/id where id is a string of
   ...
   * 
   * Uses remote `source` to create a new dataset using the arguments in `args`.
   * If `wait_time` is higher than 0 then the dataset creation request is not
   * sent until the `source` has been created successfuly.
   * 
   ...
   * @param sourceId
   *          a unique identifier in the form source/id where id is a string of
   *          24 alpha-numeric chars for the source to attach the dataset.
   * @param args
   *          set of parameters for the new dataset. Optional
   * @param waitTime
   *          time to wait for next check of FINISHED status for source before
   *          to start to create the dataset. Optional
   * @param retries
   ...
   * Retrieves a dataset.
   * 
   * GET /andromeda/dataset/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * 
   * @param datasetId
   *          a unique identifier in the form datset/id where id is a string of
   *          24 alpha-numeric chars.
   * 
   */
   ...
   * 
   * @param datasetId
   *          a unique identifier in the form dataset/id where id is a string of
   *          24 alpha-numeric chars.
   * 
   */
   ...
   * Updates a dataset.
   * 
   * PUT /andromeda/dataset/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io Content-Type: application/json
   * 
   * @param datasetId
   *          a unique identifier in the form dataset/id where id is a string of
   ...
   * @param datsetId
   *          a unique identifier in the form datset/id where id is a string of
   *          24 alpha-numeric chars for the dataset to attach the model.
   * @param args
   *          set of parameters for the new model. Optional
   * @param waitTime
   *          time to wait for next check of FINISHED status for source before
   *          to start to create the model. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   *          24 alpha-numeric chars for the dataset to attach the mdel.
   * @param args
   *          set of parameters for the new mdel. Optional
   * @param waitTime
   *          time (milliseconds) to wait for next check of FINISHED status for
   *          source before to start to create the mdel. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   * 
   * @param modelId
   *          a unique identifier in the form model/id where id is a string of
   *          24 alpha-numeric chars.
   * 
   */
   ...
   * 
   */
  public JSONObject getPublicModel(final String modelId, final String apiUser,
      final String apiKey) {
   ...
   * 
   */
  public JSONObject getModel(final JSONObject modelJSON, final String apiUser,
      final String apiKey) {
   ...
   * 
   */
  public JSONObject getPublicModel(final String modelId, final String queryString) {
   ...
   * 
   * GET /andromeda/model?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY; Host:
   * bigml.io
   * 
   * @param queryString
   *          query filtering the listing.
   ...
   * Deletes a model.
   * 
   * DELETE /andromeda/model/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1
   * 
   * @param modelId
   *          a unique identifier in the form model/id where id is a string of
   ...
   * 
   * POST /andromeda/prediction?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io Content-Type: application/json
   * 
   * @param modelOrEnsembleId
   *          a unique identifier in the form model/id or ensembke/id where id
   *          is a string of 24 alpha-numeric chars for the nodel or ensemble to
   ...
   * @param waitTime
   *          time to wait for next check of FINISHED status for model before to
   *          start to create the prediction. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
  public JSONObject createPrediction(final String modelOrEnsembleId,
      JSONObject inputData, Boolean byName, String args, Integer waitTime,
      Integer retries) {
   ...
   * 
   * @param predictionId
   *          a unique identifier in the form prediction/id where id is a string
   ...
   * 
   * POST /andromeda/evaluation?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io Content-Type: application/json
   * 
   * @param modelOrEnsembleId
   *          a unique identifier in the form model/id or ensemble/id where id
   *          is a string of 24 alpha-numeric chars for the model/ensemble to
   *          attach the evaluation.
   * @param datasetId
   *          a unique identifier in the form dataset/id where id is a string of
   *          24 alpha-numeric chars for the dataset to attach the evaluation.
   * @param args
   *          set of parameters for the new evaluation. Optional
   * @param waitTime
   *          time to wait for next check of FINISHED status for model before to
   *          start to create the evaluation. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   * 
   * An evaluation is an evolving object that is processed until it reaches the
   * FINISHED or FAULTY state, the method will return a JSONObject that encloses
   * the evaluation values and state info available at the time it is called.
   * 
   * GET
   * /andromeda/evaluation/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * 
   * @param evaluationId
   *          a unique identifier in the form evaluation/id where id is a string
   *          of 24 alpha-numeric chars.
   * 
   */
   ...
   * 
   * @param evaluationId
   *          a unique identifier in the form evauation/id where id is a string
   *          of 24 alpha-numeric chars.
   * @param changes
   ...
   * @param waitTime
   *          time to wait for next check of FINISHED status for dataset before
   *          to start to create the ensemble. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   * FINISHED or FAULTY state, the method will return a JSONObject that encloses
   * the ensemble values and state info available at the time it is called.
   * 
   * GET /andromeda/ensemble/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * 
   * @param ensembleId
   *          a unique identifier in the form ensemble/id where id is a string
   *          of 24 alpha-numeric chars.
   * 
   */
   ...
   * @param waitTime
   *          time (milliseconds) to wait for next check of FINISHED status for
   *          model before to start to create the batch prediction. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   * @param datasetsIds
   *          list of identifiers in the form dataset/id where id is a string of
   *          24 alpha-numeric chars for the dataset to attach the cluster.
   * @param args
   *          set of parameters for the new cluster. Optional
   * @param waitTime
   *          time (milliseconds) to wait for next check of FINISHED status for
   *          source before to start to create the cluster. Optional
   * @param retries
   *          number of times to try the operation. Optional
   * 
   */
   ...
   * FINISHED or FAULTY state, the method will return a JSONObject that encloses
   * the cluster values and state info available at the time it is called.
   * 
   * GET /andromeda/ensemble/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * 
   * @param clusterId
   *          a unique identifier in the form cluster/id where id is a string of
   *          24 alpha-numeric chars.
   * 
   */
   ...
   * the ensemble values and state info available at the time it is called.
   * 
   * GET /andromeda/cluster/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * 
   * @param clusterJSON
   ...
   * Updates a cluster.
   * 
   * PUT /andromeda/cluster/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io Content-Type: application/json
   * 
   * @param clusterId
   *          a unique identifier in the form cluster/id where id is a string of
 ...
 // ################################################################
   ...
   *
   * POST /andromeda/centroid?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY; HTTP/1.1
   * Host: bigml.io
   * Content-Type: application/json
   *
   * @param clusterId	a unique identifier in the form cluster/id where id is a string of 24
   * 					alpha-numeric chars for the cluster.
   * @param args		set of parameters for the new centroid. Optional
   * @param waitTime	time (milliseconds) to wait for next check of FINISHED status for centroid
   * 					before to start to create the centroid. Optional
   * @param retries		number of times to try the operation. Optional
   *
   */
  public JSONObject createCentroid(final String clusterId, JSONObject inputDataJSON, String args, Integer waitTime, Integer retries) {
	  return centroid.create(clusterId, inputDataJSON, args, waitTime, retries);
  }
   ...
   *
   * A centroid is an evolving object that is processed until it
   * reaches the FINISHED or FAULTY state, the method will return a
   * JSONObject that encloses the centroid values and state info
   * available at the time it is called.
   *
   * GET /andromeda/centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   *
   * @param centroidId 	a unique identifier in the form centroid/id where id
   ...
   * HTTP/1.1 Host: bigml.io
   *
   * @param centroid 	a centroid JSONObject.
   *
   */
   ...
   * Updates a centroid.
   *
   * PUT /andromeda/centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * Content-Type: application/json
   *
   * @param centroidId 	a unique identifier in the form centroid/id where id
   * 					is a string of 24 alpha-numeric chars.
   * @param changes		set of parameters to update the centroid. Optional
   *
   */
  public JSONObject updateCentroid(final String centroidId, final String changes) {
   ...
   * Content-Type: application/json
   *
   * @param centroid 	an centroid JSONObject
   * @param changes		set of parameters to update the centroid. Optional
   *
   */
  public JSONObject updateCentroid(final JSONObject centroidJSON, final JSONObject changes) {
   ...
   * Deletes a centroid.
   *
   * DELETE /andromeda/centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1
   *
   * @param centroidId 	a unique identifier in the form centroid/id where id is a
   * 					string of 24 alpha-numeric chars.
   *
   */
 ...
 // ################################################################
   ...
   *
   * POST /andromeda/batch_centroid?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY; HTTP/1.1
   * Host: bigml.io
   * Content-Type: application/json
   *
   * @param clusterId	a unique identifier in the form cluster/id where id is a string of 24
   ...
   * 					alpha-numeric chars for the dataset.
   * @param args		set of parameters for the new batch_centroid. Optional
   * @param waitTime	time (milliseconds) to wait for next check of FINISHED status for batch_centroid
   * 					before to start to create the batch_centroid. Optional
   * @param retries		number of times to try the operation. Optional
   *
   */
  public JSONObject createBatchCentroid(final String clusterId, final String datasetId, String args, Integer waitTime, Integer retries) {
	  return batchCentroid.create(clusterId, datasetId, args, waitTime, retries);
  }
   ...
   *
   * A batch_centroid is an evolving object that is processed until it
   * reaches the FINISHED or FAULTY state, the method will return a
   * JSONObject that encloses the batch_centroid values and state info
   * available at the time it is called.
   *
   * GET /andromeda/batch_centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   *
   * @param batchCentroidId	a unique identifier in the form batchcentroid/id where id
   * 						is a string of 24 alpha-numeric chars.
   *
   */
   ...
   * HTTP/1.1 Host: bigml.io
   *
   * @param batchCentroidJSON 	a batch_centroid JSONObject.
   *
   */
  ...
  public JSONObject downloadBatchCentroid(
      final JSONObject batchCentroidJSON, final String filename) {
    return batchCentroid.downloadBatchCentroid(batchCentroidJSON,
        filename);
  }
   ...
   *
   * GET /andromeda/batch_centroid?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * Host: bigml.io
   *
   * @param queryString	query filtering the listing.
   *
   ...
   *
   * PUT /andromeda/batch_centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1 Host: bigml.io
   * Content-Type: application/json
   *
   * @param batchCentroidId 	a unique identifier in the form batchcentroid/id where id
   ...
   *
   */
  public JSONObject updateBatchCentroid(final String batchCentroidId, final String changes) {
   ...
   * Content-Type: application/json
   *
   * @param batchCentroid 	an batch_centroid JSONObject
   * @param changes		set of parameters to update the batch_centroid. Optional
   *
   */
  public JSONObject updateBatchCentroid(final JSONObject batchCentroidJSON, final JSONObject changes) {
   ...
   *
   * DELETE /andromeda/batch_centroid/id?username=$BIGML_USERNAME;api_key=$BIGML_API_KEY;
   * HTTP/1.1
   *
   * @param batchCentroidId 	a unique identifier in the form batchCentroid/id where id is a
   * 					string of 24 alpha-numeric chars.
   *
   */
