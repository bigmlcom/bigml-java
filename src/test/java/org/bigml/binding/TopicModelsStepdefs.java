...
public class TopicModelsStepdefs {
	...
	public void I_wait_until_topic_model_status_code_is(int code1, int code2,
			int secs) throws AuthenticationException {
		Long code = (Long) ((JSONObject) context.topicModel.get("status"))
				.get("code");
	...
	public void I_update_the_topic_model_name_to(String topicModelName)
		...
		JSONObject resource = BigMLClient.getInstance()
				.updateTopicModel(topicModelId, args.toString());
		context.status = (Integer) resource.get("code");
		context.location = (String) resource.get("location");
		context.topicModel = (JSONObject) resource.get("object");
